...
class MedicationIngredientHelperTest {
    ...
    fun builtWith_shouldThrow_whenIngrNameNotProvided() {
        try {
            MedicationIngredientHelper.buildWith(null, INGREDIENT_QUANTITY, INGREDIENT_UNIT)
        ...
        try {
            MedicationIngredientHelper.buildWith("", INGREDIENT_QUANTITY, INGREDIENT_UNIT)
            fail("Exception expected")
        } catch (e: IllegalArgumentException) {
            assertThat(e.message).isEqualTo("ingredientName is required")
        }
    ...
    fun builtWith_shouldThrow_whenIngrQuantityNotProvided() {
        try {
            MedicationIngredientHelper.buildWith(INGREDIENT_NAME, null, INGREDIENT_UNIT)
            fail("Exception expected")
        } catch (e: IllegalArgumentException) {
    ...
    fun builtWith_shouldThrow_whenIngrUnitNotProvided() {
        try {
            MedicationIngredientHelper.buildWith(INGREDIENT_NAME, INGREDIENT_QUANTITY, null)
            fail("Exception expected")
        } catch (e: IllegalArgumentException) {
        ...
        try {
            MedicationIngredientHelper.buildWith(INGREDIENT_NAME, INGREDIENT_QUANTITY, "")
            fail("Exception expected")
        } catch (e: IllegalArgumentException) {
            assertThat(e.message).isEqualTo("ingredientUnit is required")
        }
